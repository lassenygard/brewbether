...
;(function ($, window, document, undefined) {
  var pluginName = "countdown360",
    defaults = {
      ...
      strokeStyle: "#808080",          // the color of the stroke
      strokeWidth: undefined,          // the stroke width, dynamically calulated if omitted in options
      fillStyle: "#ffffff",            // the fill color
      fontColor: "#bfbfbf",            // the font color
      fontFamily: "Arial",        // the font family
      fontSize: undefined,             // the font size, dynamically calulated if omitted in options
      fontWeight: 700,                 // the font weight
      autostart: true,                 // start the countdown automatically
      seconds: 10,                     // the number of seconds to count down
      label: ["minutt", "minutter"],    // the label to use or false if none
  ...
  Plugin.prototype = {
    ...
    _draw: function () {
      ...
      secondsElapsed = Math.floor((millisElapsed)/60000);
        endAngle = (Math.PI*3.5) - (((Math.PI*2)/(this.settings.seconds * 60000)) * millisElapsed);
      this._clearRect();
